...
def mask_to_tensor_1(len_list, batch_size, max_len):
    token_len = max_len
...
def mask_to_tensor_2(len_list, batch_size):
...
class collater_1():
    def __init__(self, ner2idx, rel2idx, dataset):
    ...
    def __call__(self, data):
        ...
        ner_labels = [gen_ner_labels(ners, self.max_len, self.ner2idx) for ners in ner_labels]
        rc_labels = [gen_rc_labels(rcs, self.max_len, self.rel2idx) for rcs in rc_labels]

        ner_labels = torch.stack(ner_labels, dim=2)
        rc_labels = torch.stack(rc_labels, dim=2)
        mask = mask_to_tensor_1(bert_len, batch_size, self.max_len)
...
class collater_2():
    ...
    def __call__(self, data):
        ...
        ner_labels = torch.stack(ner_labels, dim=2)
        rc_labels = torch.stack(rc_labels, dim=2)
        mask = mask_to_tensor_2(bert_len, batch_size)
...
class collater_3():
    ...
    def __call__(self, data):
        ...
        ner_labels = [item[1] for item in data]
        rc_labels = [item[2] for item in data]
        attention_masks = [item[3] for item in data]
        ...
        return [words, ner_labels, rc_labels, attention_masks, mask]
...
class collater_4():
    ...
    def __call__(self, data):
        ...
        rc_labels = [item[2] for item in data]
        bert_len = [item[3] for item in data]
        word_to_bep = [item[4] for item in data]
        ...
        return [words, ner_labels, rc_labels, mask, word_to_bep]
...
class collater_5():
    ...
    def __call__(self, data):
        ...
        ner_labels = [item[1] for item in data]
        rc_labels = [item[2] for item in data]
        word_ranges = [item[3] for item in data]
        bert_len = [item[4] for item in data]
        ...
        return [words, ner_labels, rc_labels, word_ranges, mask]
